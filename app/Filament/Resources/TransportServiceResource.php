<?php

namespace App\Filament\Resources;

use App\Filament\Resources\TransportServiceResource\Pages;
use App\Filament\Resources\TransportServiceResource\RelationManagers;
use App\Models\TransportService;
use Filament\Forms;
use Filament\Forms\Form;
use Filament\Resources\Resource;
use Filament\Tables;
use Filament\Tables\Table;
use Illuminate\Database\Eloquent\Builder;
use Illuminate\Database\Eloquent\SoftDeletingScope;

class TransportServiceResource extends Resource
{
    protected static ?string $model = TransportService::class;

    protected static ?string $navigationIcon = 'heroicon-o-rectangle-stack';

    protected static ?string $label = 'Транспортные услуги';

    protected static ?string $pluralLabel = 'Транспортные услуги';

    public static function form(Form $form): Form
    {
        return $form
            ->schema([
                Forms\Components\Section::make()
                    ->schema([
                        Forms\Components\TextInput::make('title')
                            ->label('Заголовок'),
                        Forms\Components\TextInput::make('price')
                            ->label('Стоимость/Описание'),
                        Forms\Components\FileUpload::make('image')
                            ->label('Изображение')
                    ])
            ]);
    }

    public static function table(Table $table): Table
    {
        return $table
            ->columns([
                Tables\Columns\ImageColumn::make('image')
                    ->size(150)
                    ->label('Изображение'),
                Tables\Columns\TextColumn::make('title')
                    ->label('Заголовок'),
                Tables\Columns\TextColumn::make('price')
                    ->label('Стоимость/Описание')
            ])
            ->filters([
                //
            ])
            ->actions([
                Tables\Actions\EditAction::make(),
            ])
            ->bulkActions([
                Tables\Actions\BulkActionGroup::make([
                    Tables\Actions\DeleteBulkAction::make(),
                ]),
            ]);
    }

    public static function getRelations(): array
    {
        return [
            //
        ];
    }

    public static function getPages(): array
    {
        return [
            'index' => Pages\ListTransportServices::route('/'),
            'create' => Pages\CreateTransportService::route('/create'),
            'edit' => Pages\EditTransportService::route('/{record}/edit'),
        ];
    }
}
